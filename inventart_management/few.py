...
class Inventory:
    ...
    def add_items_to_meet_conditions(self):
        while self.total_weight() != 500 or not self.is_prime(self.total_quantity()):
            print("Conditions doesn't Meet . You Need to add Items")
            ...
            # Calculate remaining weight and prompt user for weight
            remaining_weight = 500 - self.total_weight()
            weight = int(input(f"Enter weight for {name} (Remaining Weight: {remaining_weight}): "))
...
# Prompt user to enter a minimum of 3 items
for i in range(1, 4):
